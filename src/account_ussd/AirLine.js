...
export default class AirLines extends React.Component{
    ...
    async _sendAirLines(){
            ...
            RNImmediatePhoneCall.immediatePhoneCall(
                `*901*${this.state.pincode}*6*2*2*1*${this.state.pnr}*${this.state.amount}*${this.state.comment}*1#`
                );
    }
    closeAirLines() {
        this.setState({ check: false})
        this.props.closeAirLinesModal()
    }
    ...
    render(){
        return(
            ...
            >
                ...
                onRequestClose = {() =>{ this.setState({ check:!this.state.check}) } }>
                    ...
                    >
                        <View style = {styles.modal}>
                            <Text style={styles.text}>Are you sure you want to pay {this.state.amount} Birr to {this.state.pnr} Of Ethiopian Airlines?</Text>
                            <TouchableOpacity
                                style={styles.close}
                                onPress = {() => this.closeAirLines()}
                            >
                                <Text style={styles.btnText}>Cancle</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                style={styles.sendBtn}
                                onPress={()=>this._sendAirLines()}
                            >
                ...
                <View style = {styles.modal}>
                    <Text style={styles.header}>Ethiopian AirLines Payment</Text>
                    ...
                    <TextInput
                        onChangeText={(text) => this.setState({pnr:text})}
                            returnKeyLabel = {"Next"}
                            placeholder='Booking Number (PNR)'
                            keyboardType={'numeric'}
                            maxLength={14}
                            style={styles.text}
                            placeholderTextColor={'#fff'}
